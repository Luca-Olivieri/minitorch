...
void test_tensors_with_dims0() {
    ...
    ASSERT_THROWS(Tensor t({0}), std::runtime_error);
    ASSERT_THROWS(Tensor t({4, 0}), std::runtime_error);
}
void test_negative_tensor_indices() {
    Tensor t({2, 3, 4});
    
    // ASSERT_THROWS(std::runtime_error, t[{-1}]);
    // t[nullptr];
}
